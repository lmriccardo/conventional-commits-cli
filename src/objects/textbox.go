...
func (tb *TextBox) getStringPosition() int {
	row_size := tb.getMaxRowSize()                       // Max row size
	rel_cursor_x := tb.curr_pos_x - tb.start_pos_x       // Relative X cursor position
	str_position := tb.curr_line*row_size + rel_cursor_x // Position into the string
	return str_position
}
...
func (tb *TextBox) handleBackspace(screen tcell.Screen) {
	...
	str_position := tb.getStringPosition() // Take the position into the string
	content_array := []rune(tb.content)    // From string to rule array
	content_len := len(tb.content)         // Take the length of the string
